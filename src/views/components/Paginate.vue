<script setup>
    const { prevBox, nextBox, currentBox } = defineProps({
        currentBox: Number,
        prevBox: Function,
        nextBox: Function,
    })
...
<template>
    <div class="flex m-4 w-full items-center justify-center transition-all duration-300">
        <div class="flex w-1/2 py-3 items-center justify-between bg-slate-100 rounded-full">
            <span class="cursor-scale flex items-center justify-center p-2 w-20 max-h-full text-red-500 hover:scale-[2] transition-all duration-200" @click="prevBox">
            ...
            <span class="w-full text-center text-xl lg:text-2xl mx-4">{{ 'Box ' + currentBox }}</span>
            <span class="cursor-scale flex items-center justify-center p-2 w-20 h-full text-red-500 hover:scale-[2] transition-all duration-200" @click="nextBox">
